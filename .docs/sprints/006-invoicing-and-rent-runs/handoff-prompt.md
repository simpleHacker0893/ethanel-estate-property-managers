...
We are starting **Sprint 006 — Invoicing and rent runs**. Services touched: `money-svc`, `money-worker`, `docs-svc`, `docs-worker`.
...
6. `.docs/sprints/006-invoicing-and-rent-runs/requirements.md`
7. `.docs/sprints/006-invoicing-and-rent-runs/blueprint.md`
8. `.docs/sprints/006-invoicing-and-rent-runs/acceptance.md`

Then, before writing code:

```bash
graphify query "what already exists for invoicing and rent runs?"
```
...
1. A 500-unit run on the staging seed finishes inside the D-40 budget.
2. Running the same rent run twice creates no duplicate invoices and no duplicate postings.
3. An invoice, a receipt and a statement render as PDFs that a partner would send to a landlord unedited.

These are restated in `.docs/sprints/006-invoicing-and-rent-runs/acceptance.md` with an evidence
...
Review the work against `.docs/sprints/006-invoicing-and-rent-runs/acceptance.md` and report:
