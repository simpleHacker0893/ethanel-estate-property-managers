...
We are starting **Sprint 014 — Reliability and operations**. Services touched: `all`.
...
6. `.docs/sprints/014-hardening-reliability/requirements.md`
7. `.docs/sprints/014-hardening-reliability/blueprint.md`
8. `.docs/sprints/014-hardening-reliability/acceptance.md`

Then, before writing code:

```bash
graphify query "what already exists for reliability and operations?"
```
...
1. Every failure mode that has occurred in staging has a runbook written from a real reproduction.
2. The combined load test passes with both workloads running together.
3. An SLO breach pages your phone within the stated detection window.

These are restated in `.docs/sprints/014-hardening-reliability/acceptance.md` with an evidence
...
Review the work against `.docs/sprints/014-hardening-reliability/acceptance.md` and report:
