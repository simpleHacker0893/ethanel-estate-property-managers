...
We are starting **Sprint 002 — The chassis and tenancy**. Services touched: `packages/chassis`, `charts/service`, `identity-svc`.
...
6. `.docs/sprints/002-chassis-and-tenancy/requirements.md`
7. `.docs/sprints/002-chassis-and-tenancy/blueprint.md`
8. `.docs/sprints/002-chassis-and-tenancy/acceptance.md`

Then, before writing code:

```bash
graphify query "what already exists for the chassis and tenancy?"
```
...
1. Scaffolding a brand new service - schema, role, contract, job handlers, loaded by an existing Deployment - and getting it to staging takes under one day. If it does not, this sprint is not done - see RISKS R-01 tripwire.
2. Moving a service into its own Deployment has been done once, to prove D-54's split path is a values.yaml and a connection string rather than a claim.
3. A user signs in with Google, lands in their organization, and is provably unable to reach another organization's data via API or direct query.
4. A pull request adding a tenant-scoped table without an RLS policy fails CI.

These are restated in `.docs/sprints/002-chassis-and-tenancy/acceptance.md` with an evidence
...
Review the work against `.docs/sprints/002-chassis-and-tenancy/acceptance.md` and report:
