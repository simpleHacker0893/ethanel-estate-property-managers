...
- charts/service Helm library chart: probes, CPU/memory requests and limits, PodDisruptionBudget, non-root with read-only root filesystem, default-deny NetworkPolicy with an explicit allow list. Every D-34 gate becomes structural. Four Deployments inherit it (web, gateway, worker, docs-worker) per D-54, and adding a fifth must stay a values file.
