...
We are starting **Sprint 004 — The grid and the readiness floor**. Services touched: `web`, `platform`.
...
6. `.docs/sprints/004-grid-and-readiness-floor/requirements.md`
7. `.docs/sprints/004-grid-and-readiness-floor/blueprint.md`
8. `.docs/sprints/004-grid-and-readiness-floor/acceptance.md`

Then, before writing code:

```bash
graphify query "what already exists for the grid and the readiness floor?"
```
...
1. The grid does every behaviour in the grid requirements on real partner data.
2. You have restored the database from backup once, on the clock, with the steps written down.
3. The staging seed is large enough to make a 500-unit rent run meaningful in Sprint 006.

These are restated in `.docs/sprints/004-grid-and-readiness-floor/acceptance.md` with an evidence
...
Review the work against `.docs/sprints/004-grid-and-readiness-floor/acceptance.md` and report:
