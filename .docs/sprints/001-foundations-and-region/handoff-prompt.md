...
We are starting **Sprint 001 — Ground truth and the pipeline**. Services touched: `web (skeleton)`, `identity-svc (hello-world)`, `infrastructure`.
...
6. `.docs/sprints/001-foundations-and-region/requirements.md`
7. `.docs/sprints/001-foundations-and-region/blueprint.md`
8. `.docs/sprints/001-foundations-and-region/acceptance.md`

Then, before writing code:

```bash
graphify query "what already exists for ground truth and the pipeline?"
```
...
1. A commit on main is running in the staging namespace with no manual step.
2. The Neon region is decided from measured numbers and recorded in ADR-002.
3. Partner sample files are committed as anonymised fixtures.
4. The caretaker constraints document exists and names specific devices.

These are restated in `.docs/sprints/001-foundations-and-region/acceptance.md` with an evidence
...
Review the work against `.docs/sprints/001-foundations-and-region/acceptance.md` and report:
