...
We are starting **Sprint 003 — The property domain**. Services touched: `property-svc`, `web shell`.
...
6. `.docs/sprints/003-property-domain/requirements.md`
7. `.docs/sprints/003-property-domain/blueprint.md`
8. `.docs/sprints/003-property-domain/acceptance.md`

Then, before writing code:

```bash
graphify query "what already exists for the property domain?"
```
...
1. A partner's smallest building is fully represented - properties, units, landlords, residents, leases - entered by hand.
2. The domain model has been checked against the partner's real lease and rent roll from Sprint 001 and survived without a field being forced.

These are restated in `.docs/sprints/003-property-domain/acceptance.md` with an evidence
...
Review the work against `.docs/sprints/003-property-domain/acceptance.md` and report:
