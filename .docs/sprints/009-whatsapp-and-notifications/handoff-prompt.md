...
We are starting **Sprint 009 — WhatsApp and notifications**. Services touched: `messaging-svc`, `messaging-worker`.
...
6. `.docs/sprints/009-whatsapp-and-notifications/requirements.md`
7. `.docs/sprints/009-whatsapp-and-notifications/blueprint.md`
8. `.docs/sprints/009-whatsapp-and-notifications/acceptance.md`

Then, before writing code:

```bash
graphify query "what already exists for whatsapp and notifications?"
```
...
1. A resident pays and receives a WhatsApp receipt generated from the ledger entry, not from the callback.
2. Two staff cannot reply to the same conversation; the second sees it read-only.
3. A resident can turn off marketing notifications and cannot turn off a rent invoice.

These are restated in `.docs/sprints/009-whatsapp-and-notifications/acceptance.md` with an evidence
...
Review the work against `.docs/sprints/009-whatsapp-and-notifications/acceptance.md` and report:
