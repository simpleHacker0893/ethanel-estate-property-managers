...
We are starting **Sprint 012 — Land inventory, God's-eye view, billing**. Services touched: `listing-svc`, `billing-svc`.
...
6. `.docs/sprints/012-land-plots-and-billing/requirements.md`
7. `.docs/sprints/012-land-plots-and-billing/blueprint.md`
8. `.docs/sprints/012-land-plots-and-billing/acceptance.md`

Then, before writing code:

```bash
graphify query "what already exists for land inventory, god's-eye view, billing?"
```
...
1. A land company is onboarded end to end and can market and sell a plot on instalments.
2. Every pilot-risky feature is behind a flag you can flip per organization without a deploy.
3. Usage counters produce the number a manual invoice would be based on.

These are restated in `.docs/sprints/012-land-plots-and-billing/acceptance.md` with an evidence
...
Review the work against `.docs/sprints/012-land-plots-and-billing/acceptance.md` and report:
