...
We are starting **Sprint 007 — Payments and reconciliation**. Services touched: `payments-svc`, `payments-worker`, `gateway`.
...
6. `.docs/sprints/007-payments-and-reconciliation/requirements.md`
7. `.docs/sprints/007-payments-and-reconciliation/blueprint.md`
8. `.docs/sprints/007-payments-and-reconciliation/acceptance.md`

Then, before writing code:

```bash
graphify query "what already exists for payments and reconciliation?"
```
...
1. The burst test passes: 50/s for 10 minutes, zero loss.
2. The measured auto-match rate against the partners' real M-Pesa statements is published, whatever it is.
3. If the rate is under 95%, the causes are written into RISKS.md and the fix is scoped into Sprint 015.
4. A real shilling has moved through staging end to end.

These are restated in `.docs/sprints/007-payments-and-reconciliation/acceptance.md` with an evidence
...
Review the work against `.docs/sprints/007-payments-and-reconciliation/acceptance.md` and report:
