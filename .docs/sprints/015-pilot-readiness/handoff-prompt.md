...
We are starting **Sprint 015 — Pilot readiness**. Services touched: `web`, `payments-svc`, `money-svc`.
...
6. `.docs/sprints/015-pilot-readiness/requirements.md`
7. `.docs/sprints/015-pilot-readiness/blueprint.md`
8. `.docs/sprints/015-pilot-readiness/acceptance.md`

Then, before writing code:

```bash
graphify query "what already exists for pilot readiness?"
```
...
1. The 95% auto-match bar is met on real partner data.
2. One partner's entire portfolio has been imported with no manual spreadsheet editing.
3. Every hard-to-reverse decision in DECISIONS.md has an ADR.
4. Usability fixes from the usability tests are merged, not backlogged.

These are restated in `.docs/sprints/015-pilot-readiness/acceptance.md` with an evidence
...
Review the work against `.docs/sprints/015-pilot-readiness/acceptance.md` and report:
