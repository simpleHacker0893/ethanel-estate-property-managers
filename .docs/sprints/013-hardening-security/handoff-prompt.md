...
We are starting **Sprint 013 — Security, isolation and obligations**. Services touched: `all`, `gateway`.
...
6. `.docs/sprints/013-hardening-security/requirements.md`
7. `.docs/sprints/013-hardening-security/blueprint.md`
8. `.docs/sprints/013-hardening-security/acceptance.md`

Then, before writing code:

```bash
graphify query "what already exists for security, isolation and obligations?"
```
...
1. The D-34 gate list is green in CI output rather than in someone's opinion.
2. E3 coverage is 100% of tenant-scoped tables and endpoints, with the coverage number published.
3. A data subject access request can be fulfilled end to end by following a written procedure.

These are restated in `.docs/sprints/013-hardening-security/acceptance.md` with an evidence
...
Review the work against `.docs/sprints/013-hardening-security/acceptance.md` and report:
